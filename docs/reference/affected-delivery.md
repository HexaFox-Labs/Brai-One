...
обязательный commit status `runtime-acceptance`. Затем главный авторизованный
агент, а не Сергей вручную, запрашивает squash-merge с точным head SHA. GitHub
branch protection отклонит merge, если появился новый commit или check снова
стал красным. `GITHUB_TOKEN` сам merge не выполняет: такие merge подавляют
обычные downstream events GitHub и могли бы пропустить Dev delivery/cleanup.
Для документации принятие и preview не требуются: delivery сам выставляет
status «Preview не требуется», а главный агент сливает PR после reduced checks.
...
Preview manifests всех ещё не доставленных merge-коммитов и выбирает самый
новый доступный. Канонический controller manifest хранит для каждого image
сразу строку `ghcr.io/...@sha256:...`; Dev валидирует именно эту форму и
переносит нужные OCI digests. Если связь или manifest отсутствует, pipeline не
угадывает и безопасно выполняет обычную affected-сборку.
...
последнего доставленного Dev до своего head, найдёт принятый Preview внутри
этого диапазона и не потеряет runtime-изменение. То же правило действует после
неудачного промежуточного Dev run. Release всегда сравнивается со своим frozen
Dev merge-base.
...
Control-класс добавляет консервативные проверки, но не вычитает найденный Nx
runtime: смешанный диапазон `workflow/config + web` по-прежнему строит или
переиспользует web image и требует runtime delivery.
