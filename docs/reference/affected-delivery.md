...
| merge в `dev`                                                 | Только нужные `d-brai-*`; exact Preview digests переиспользуются, иначе — affected build. Затем fresh snapshot.                                                |
...
включается нативный GitHub auto-merge (squash): GitHub сам дождётся всех обязательных
...
4. Первое защищённое GitHub Actions `bootstrap_dev` на `dev` публикует полный
   digest set, разворачивает `d-brai-*`, применяет Factory baseline, затем
   one-time Access foundation и только после этого least-privilege Access роли;
   пароль и audit migration-роли завершаются до её миграций. Затем controller
   ждёт healthchecks и делает snapshot.
