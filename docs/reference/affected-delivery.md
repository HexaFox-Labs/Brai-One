...
GHCR packages можно держать public, как и репозиторий: образ не содержит
секретов, а сервер принимает исключительно expected-path digest references и
не использует tags. Это избегает хранения широкого GitHub token на сервере.
Если package visibility сменят на private, нужно до первого deploy отдельно
установить только package-read credential; controller не использует личный
GitHub token автоматически.
