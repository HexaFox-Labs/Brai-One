...
Deployment, registry and preview credentials SHALL be available only to trusted
primary-repository workflows with the minimum required permission and a
protected-environment gate where the action can affect a live environment. A
public fork, issue, comment, arbitrary dispatch input or untrusted workflow
artifact MUST NOT obtain a delivery credential or invoke a host deployment
command.
